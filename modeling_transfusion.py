# Based on transformer's modelling_llama.py
...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(
        ...
        scaling_factor=1.0,
        rope_type="default",
        config: Optional[TransfusionConfig] = None,
    ):
        super().__init__()
...
class TransfusionAttention(nn.Module):
    ...
    def __init__(self, config: TransfusionConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        self.block_mask = create_block_mask(self.transfuser_mask, B=None, H=None, Q_LEN=0, KV_LEN=0)
...
TRANSFUSION_ATTENTION_CLASSES = {
    "eager": TransfusionAttention,
}
...
class TransfuserDecoderLayer(nn.Module):
    def __init__(self, config: TransfusionConfig, layer_idx: int):
        ...
        self.self_attn = TRANSFUSION_ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)
